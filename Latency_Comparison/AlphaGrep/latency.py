...
def compare_metrics(df1, df2, label1, label2, output_dir):
    ...
    for metric in metrics:
        for perc in percentiles:
            ...
            comparison_rows.append({
                "Metric": metric,
                "Percentile": perc,
                f"{label1}": round(val1, 2),
                f"{label2}": round(val2, 2),
                "Difference": round(val2 - val1, 2),
                "Percent Change": round(((val2 - val1) / val1 * 100), 2) if val1 != 0 else float('inf')
            })

    full_percentile_df = pd.DataFrame(comparison_rows)
    full_percentile_csv = os.path.join(output_dir, "percentile_comparison_full.csv")
    full_percentile_df.to_csv(full_percentile_csv, index=False)
    print(f"✅ Full percentile-wise comparison table saved to: {full_percentile_csv}\n")
    print(full_percentile_df.to_string(index=False))

    # --- Optional: Plotting code still remains unchanged ---
    for metric in metrics:
        df_metric = full_percentile_df[full_percentile_df['Metric'] == metric].copy()
    ...
    # --- Smoothed Intraday Plot (optional) ---
    for metric in metrics:
...
def main():
    ...
    # Use full filename without extension (preserve all dots)
